...
function randomizedSelect(arr, low, high, k, comp) {
    ...
    if (k === j) return arr[i];else if (k < j) return randomizedSelect(arr, low, i - 1, k, comp);else return randomizedSelect(arr, i + 1, high, k - j, comp);
}
...
(function () {
    ...
    function tournament(n) {
        ...
        function makeCopy(table, n) {
            ...
            if (k > 1 && isOdd(k)) copyOdd(table, n);else copy(table, n);
        }
...
function specifiedBinarySearch(arr, x) {
    ...
    while (low <= high) {
        ...
        if (arr[middle] === x) return middle;else if (arr[middle] > x) high = middle - 1;else low = middle + 1;
    }
...
(function () {
    ...
    function binarySearch4Merge(arr, x, low, high) {
        while (low <= high) {
            var middle = low + high >> 1;
            if (x === arr[middle]) return middle;else if (x < arr[middle]) high = middle - 1;else low = middle + 1;
        }

        if (x > arr[middle]) return middle;else return middle - 1;
    }
...
(function () {
    ...
    function getTwoMaxElems(arr, s, n) {
        ...
        // 如果是三个元素，做比较，选做出最大两个
                ...
                if (arr[s] > arr[s + 1]) swap(arr, s, s + 1);

                if (arr[s] < arr[n]) arr[s] = arr[n];
        ...
        var k = (n + 1 + s) / 2 | 0;
        for (var i = s; i < k; ++i) {
            if (arr[i] > arr[i + k]) swap(arr, i, i + k);
        }
...
(function () {
    ...
    function weightedMedian(arr) {
        ...
        if (c < 0.5) return 0;else return arr[i];
    }

    var arr = [{ elem: 5, weight: 0.2 }, { elem: 4, weight: 0.3 }, { elem: 3, weight: 0.2 }, { elem: 2, weight: 0.2 }, { elem: 1, weight: 0.1 }];
    ...
    function weightedMedian2(arr, i, j, sum1, sum2) {
        ...
        if (c1 > sum1) return weightedMedian2(arr, i, m - 1, sum1, sum2 - c2 - arr[m].weight);
        if (c2 > sum2) return weightedMedian2(arr, m + 1, j, sum1 - c1 - arr[m].weight, sum2);
...
function preOrder(inOrderStr, postOrderStr) {
    ...
    void function recurse(a, b) {
        if (b.length === 1) pre += b;else {
            var k = a.indexOf(b[b.length - 1]);
            pre += a[k];
            if (k > 0) recurse(a.substr(0, k), b.substr(0, k));
            if (k < a.length - 1) recurse(a.substr(k + 1, a.length - k - 1), b.substr(k, b.length - k - 1));
        }
...
function postOrder(inOrderStr, preOrderStr) {
    ...
    void function recurse(a, b) {
        if (b.length === 1) post += b;else {
            var k = a.indexOf(b[0]);
            if (k > 0) recurse(a.substr(0, k), b.substr(1, k));
            if (k < a.length - 1) recurse(a.substr(k + 1, a.length - k - 1), b.substr(k + 1, b.length - k - 1));
            post += a[k];
        }
...
function uniqueComp(n) {
    ...
    for (var i = 1; i <= n; ++i) {
        ...
        for (var i = 1; i <= n / 2; ++i) {
            count += comp(i);
            if (i > 10 && 2 * i / 10 <= i % 10) count -= arr[Math.floor(i / 10)];
        }
