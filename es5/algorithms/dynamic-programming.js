...
(function () {
    ...
    function divide_comb(n, m) {
        if (m === 0 || n === m) return 1;else return divide_comb(n - 1, m) + divide_comb(n - 1, m - 1);
    }
...
(function () {
    ...
    function LISdyna(arr) {
        ...
        for (var i = 1; i < n; ++i) {
            var k = 0;
            for (var j = 0; j < i; ++j) {
                if (arr[j] <= arr[i] && k < b[j]) k = b[j];else k = 0;
            }
            b[i] = k + 1;
        }
    ...
    function LISdyna2(arr) {
        ...
        for (var i = 1; i < arr.length; ++i) {
            if (arr[i] >= b[k]) b[++k] = arr[i];else b[binary(i, k)] = arr[i];
        }
        return k;
        ...
        function binary(i, k) {
            ...
            var j = k;
            for (var h = 1; h !== j - 1;) {
                if (b[k = Math.floor((h + j) / 2)] <= arr[i]) h = k;else j = k;
            }
...
(function () {
    ...
    function ackermann1(m, n) {
        if (m === 0) return n + 1;
        if (n === 0) return ackermann1(m - 1, 1);else return ackermann1(m - 1, ackermann1(m, n - 1));
    }
    ...
    function ackm_nonRecursive(m, n) {
        ...
        while (top) {
            ...
            if (m === 0) cache[top][2] = n + 1;else if (n === 0) {
                ++top;
                cache[top] = cache[top] || [];
                cache[top][1] = m - 1;
                cache[top][2] = 1;
...
(function () {
    ...
    function maxSubSum3(arr, n) {
        return function maxSubSum(arr, left, right) {
            var sum = 0;
            if (left === right) sum = arr[left] > 0 ? arr[left] : 0;else {
...
console.log('taskScheduling: ' + taskScheduling(6, [2, 5, 7, 10, 5, 2], [3, 8, 4, 11, 3, 4]));
...
(function () {
    ...
    function maximumDisjointSubsets(arr) {
        ...
        for (var j = arr[1]; j <= n; ++j) {
            size[1][j] = 1;
        }for (var i = 2; i < n; ++i) {
            size[i] = [];
            for (var j = 0; j < arr[i]; ++j) {
                size[i][j] = size[i - 1][j] || 0;
...
function flowShop(a, b) {
    ...
    var k = n - 1;
    for (var i = 0; i < n; ++i) {
        if (d[i].job) c[j++] = d[i].index;else c[k--] = d[i].index;
    }
...
(function () {
    ...
    function makingChange(t, l) {
        ...
        for (var j = 1; j <= l; ++j) {
            if (j % t[0] === 0) c[j] = j / t[0];else c[j] = Inifity;
        }
...
var m1 = [[0, 0, 0, 0], [0, 3, 0, 5], [0, 0, -1, 0], [0, 2, 0, 0]];
var m2 = [[0, 0, 0, 0], [0, 0, 2, 0], [0, 1, 0, 0], [0, -2, 4, 0]];
