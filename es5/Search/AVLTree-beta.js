...
AVLNode.prototype = {
     ...
     */
    leftBalance: function leftBalance(isDelete) {
     ...
     * @returns {AVLNode} 返回新的根结点
     */
    rightBalance: function rightBalance(isDelete) {
    ...
    search: function search_nonRecurse(key) {
        ...
        while (p && (cmp = AVLNode.cmp(key, p.data) !== 0)) {
            if (cmp < 0) p = p.leftChild;else p = p.rightChild;
        }

        if (!p || AVLNode.cmp(key, p.data) !== 0) return null;else return p;
    },

    insert_nonRecursive: function insert_nonRecursive(elem) {
        if (this.data == null) {
            this.data = elem;
        ...
        // s插入到q结点的左或右子树
        if (AVLNode.cmp(s.data, q.data) < 0) q.leftChild = s;else q.rightChild = s;
        ...
        if (a.balanceFactor === 2) {
            b = a.leftChild;
            if (b.balanceFactor === LH) p = a.rotate_LL(true);else p = a.rotate_LR();
        } else {
            b = a.rightChild;
            if (b.balanceFactor === LH) p = a.rotate_RL();else p = a.rotate_RR(true);
        }
        ...
        } else if (f.leftChild == a) f.leftChild = p;else f.rightChild = p;
    },
     ...
     */
    remove_Recursive: function remove_Recursive(elem, parent) {
     ...
     * @returns {*}
     */
    remove_nonRecursive: function remove_nonRecursive(elem) {
        if (this.data == null) return {
            success: false,
            data: null
        };
        ...
        while (p) {
            ...
            if (p.balanceFactor === EH || p.balanceFactor === LH && cmp > 0 && p.leftChild.balanceFactor === EH || p.balanceFactor === RH && cmp < 0 && p.rightChild.balanceFactor === EH) a = p;
        ...
        if (p.leftChild && p.rightChild) {
            ...
            while (m2) {
                ...
                if (m2.balanceFactor === EH || m2.balanceFactor === LH && cmp > 0 && m2.leftChild.balanceFactor === EH || m2.balanceFactor === RH && cmp < 0 && m2.rightChild.balanceFactor === EH) a = m2;

                if (cmp < 0) m2 = m2.leftChild;else m2 = m2.rightChild;
            }
            ...
            // 更改父结点的平衡因子
            if (pos === 'leftChild') --q.balanceFactor;else ++q.balanceFactor;
            // 删除，如果右子树就替换
        ...
        // 只有左子树或右子树的情况，直接替换
                ...
                if (q) {
                    if (q.leftChild === p) --q.balanceFactor;else ++q.balanceFactor;
                }
            }
            // 没有子树的情况，考虑根结点的情况
            else {
        ...
        if (q) {
            ...
            while (p) {
                ...
                if (AVLNode.cmp(q.data, p.data) < 0) p = p.leftChild;else p = p.rightChild;
            }
        }
...
Node.cmp = function (a, b) {
    if (a > b) return RIGHT;else if (a === b) return NEITHER;else return LEFT;
};
Node.prototype = {
    constructor: Node,
    ...
    rotate_2: function rotate_2(dir) {
    ...
    rotate_3: function rotate_3(dir, thirdDir) {
        var b = this.copy(function (a, b) {
            a.longer = b.longer;
        });
        var f = b.next[dir];
    ...
    rebalancePath: function rebalancePath(elem) {
        var path = this;
    ...
    rebalanceInsert: function rebalanceInsert(elem) {
        var path = this;
        var first, second, third;

        if (path.longer === NEITHER) ;else if (path.longer !== (first = Node.cmp(elem, path.data))) {
            path.longer = NEITHER;
            ...
            if (elem === path.data) third = NEITHER;else third = Node.cmp(elem, path.data);
    ...
    // todo bug exists
    rebalanceDel: function rebalanceDel(elem, targetp) {
        var treep = this;
        var targetn = targetp;
    ...
    checkDepth: function checkDepth() {
...
/*删除算法都有问题。。不想在这里纠结太久，以后有空再看吧。。先去研究红黑树*/