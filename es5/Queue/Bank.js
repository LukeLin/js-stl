...
Bank.prototype = {
    cmp: function cmp(event1, event2) {
        if (event1.occurTime < event2.occurTime) return -1;else if (event1.occurTime === event2.occurTime) return 0;else return 1;
    },
    // 初始化操作
    openForDay: function openForDay() {
        // 初始化累计时间和客户数为0
        this.totalTime = 0;
        this.customerNum = 0;
        // 初始化事件链表
    ...
    // 处理客户到达事件
    customerArrived: function customerArrived(durtime, intertime) {
        ++this.customerNum;
        ...
        this.queues[minQueueIndex].enQueue(new QueueElemType(this.event.occurTime, durtime));
    ...
    // 处理客户离开事件
    customerDeparture: function customerDeparture(type) {
        // 删除第i队列的排头客户
        var i = type - 1 || 0;
    ...
    simulation: function simulation(closeTime) {
        this.closeTime = closeTime || 0;
        this.openForDay();
        while (this.eventList.head) {
            var elem = this.eventList.delFirst().data;
            if (elem.eventType === 0) this.customerArrived();else this.customerDeparture(elem.eventType);
        }
